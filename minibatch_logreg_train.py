...
def minibatch_gradient_descent(data, vals, theta, learning_rate: float, iterations: int, batch_size: int = 32):
	...
	for i in range(iterations):
		...
		for n in range(0, size, batch_size):
			x_i = x[n: n + batch_size]
			y_i = y[n: n + batch_size]
			sigmoid = sigmoid_function(x_i @ theta)
			gradient = (1 / len(y_i)) * np.dot(x_i.T, (sigmoid - y_i))
...
if __name__ == '__main__':
	...
	for i, house in enumerate(houses):
		...
		theta, cost = minibatch_gradient_descent(data, vals, all_thetas[i], learning_rate, iterations)
		all_thetas[i] = theta
	...
	fig.tight_layout()
	plt.savefig('plots/minibatch_cost.png')

	# SAVE RESULTS
	output_dir = 'datasets'
	os.makedirs('datasets', exist_ok=True)
	output_path = os.path.join('datasets', 'minibatch_logreg_thetas.pkl')
